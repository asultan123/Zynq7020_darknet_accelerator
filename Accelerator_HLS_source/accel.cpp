...
#pragma HLS ARRAY_PARTITION variable=C cyclic factor=50 dim=1
 // second image
    ...
    if (control)
    {
				LOAD_A_2: for (int j = 0; j < w_col; j+=DATA_PER_PACKET) {
					if(j < w_col_sig)
					{
        ...
        // Iterate over batch elements
            L1: for (int i = 0; i < i_1_row; i+=DATA_PER_PACKET) {
    ...
    else
    {
                ...
                LOAD_C_1: for (int i = 0; i < i_1_row; i++) {
                    tmp[i] = 0;
                    LOAD_C_2: for (int j = 0; j < i_col; j+=DATA_PER_PACKET) {
#pragma HLS PIPELINE
                    	if(j < w_col_sig)
							{
								converter.packet = pop_stream(in_stream[is_idx++]);
				...
				// Iterate over output classes
				L5: for (int j = 0; j < w_col ; j++) {
					if(j>=w_col_sig)
					{
						break;
					}
					alpha = A[j];
					L6: for(int k = 0; k < i_1_row; k++) {
#pragma HLS PIPELINE
#pragma HLS UNROLL
				...
				L7: for (int k = 0; k < i_1_row; k+=DATA_PER_PACKET) {
