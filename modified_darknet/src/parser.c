...
network parse_network_cfg_custom(char *filename, int batch, int time_steps)
{
    ...
    if (gpu_index >= 0)
    {
        ...
        if (workspace_size) {
            ...
            printf("CALLED WORKSPACE PARSE %d\n", call_count++);

            int workspace_size_core_adjusted =  (int)ceil((float)workspace_size/(float)IP_BATCH_SIZE)*IP_BATCH_SIZE;

            // printf("WORKSPACE SIZE: %d ADJUSTED: %d\n", workspace_size, workspace_size_core_adjusted);

            net.workspace = (float*)cma_alloc(workspace_size_core_adjusted*4,1);
