...
void conv_layer_1(float* A, float* B, float* C)
{
    ...
    int output_offset = 150;
    int layer_workspace = (int)ceil((float)12544/(float)IP_BATCH_SIZE)*IP_BATCH_SIZE;
    int num_of_batches = layer_workspace/IP_BATCH_SIZE;
    int filter_offset = IP_BATCH_SIZE*(num_of_batches-1)+12544%IP_BATCH_SIZE;
    ...
    for(t = 0; t<num_of_filters; t++){ //filter select
        ...
        dma_transaction((weight_matrix+t*w_col), weight_recv_buf, \
            w_col, i_row, \
        ...
        for(i = 0; i<num_of_batches; i++)
        {
            ...
            {
                dma_transaction((image_matrix+ i * i_row * w_col), C+(i*output_offset)+t*filter_offset, \
                    (i_row*w_col), i_row, \
                    IP_SEND_DATA, w_col, i_row);
            }
            else
            {
                if(t < num_of_filters - 1)
                {
                    dma_transaction((image_matrix+ i * w_col * i_row), C+(i*output_offset)+ t*filter_offset, \
                    (i_row*w_col), i_row, \
...
void conv_layer_2(float* A, float* B, float* C)
{
    ...
    int output_offset = 150;
    int layer_workspace = (int)ceil((float)3136/(float)IP_BATCH_SIZE)*IP_BATCH_SIZE;
    int num_of_batches = layer_workspace/IP_BATCH_SIZE;
    int filter_offset = IP_BATCH_SIZE*(num_of_batches-1)+3136%IP_BATCH_SIZE;
...
void conv_layer_6(float* A, float* B, float* C)
{
    ...
    int num_of_filters = 32;
    int output_offset = 150;
    int layer_workspace = (int)ceil((float)784/(float)IP_BATCH_SIZE)*IP_BATCH_SIZE;
    int num_of_batches = layer_workspace/IP_BATCH_SIZE;
    int filter_offset = IP_BATCH_SIZE*(num_of_batches-1)+784%IP_BATCH_SIZE;
...
void conv_layer_10(float* A, float* B, float* C)
{
    ...
    int w_col = 256;
    int i_row = 150;
    int num_of_filters = 64;
    int output_offset = 150;
    int layer_workspace = (int)ceil((float)196/(float)IP_BATCH_SIZE)*IP_BATCH_SIZE;
    int num_of_batches = layer_workspace/IP_BATCH_SIZE;
    int filter_offset = IP_BATCH_SIZE*(num_of_batches-1)+196%IP_BATCH_SIZE;
    ...
    float *C, int ldc)
{
    ...
    if(conv_num == 0)
    {
        ...
        init_ip_params_size(IP_SEND_WEIGHTS, 144, 150);
        ip_start();
        dma_reset();
        dma_stop();
        dma_start();
    ...
    unsigned int phy_weight = cma_get_phy_addr((void*)weight_matrix);
    unsigned int phy_image = cma_get_phy_addr((void*)image_matrix);
    unsigned int phy_output = cma_get_phy_addr((void*)C);
    unsigned int size_weight = M*K;
    unsigned int size_output = (int)ceil((float)N/(float)IP_BATCH_SIZE)*IP_BATCH_SIZE*M;
    unsigned int size_image = (int)ceil((float)N/(float)IP_BATCH_SIZE)*IP_BATCH_SIZE*K;
    
    cma_flush_cache((void*)image_matrix, phy_image, size_image * 4);
    cma_invalidate_cache((void*)image_matrix, phy_image, size_image * 4);

    cma_flush_cache((void*)weight_matrix, phy_weight, size_weight * 4);
    cma_invalidate_cache((void*)weight_matrix, phy_weight, size_weight * 4);

    cma_flush_cache((void*)C, phy_output, size_output * 4);
    cma_invalidate_cache((void*)C, phy_output, size_output * 4);
    // usleep(5000);

    // printf("FLUSHED %f\n", image_matrix[0]);
    // printf("FLUSHED %f\n", weight_matrix[0]);

    if(conv_num != 1 && conv_num != 2 && conv_num != 3 && conv_num != 4 && conv_num != 5 && conv_num != 6 && conv_num != 7 && conv_num != 8 && conv_num != 9 && conv_num != 10 && conv_num != 11 && conv_num != 12 && conv_num != 13 && conv_num != 14 && conv_num != 15)
    {
    ...
    time_accumulate += (double)(end-begin)/(double)CLOCKS_PER_SEC;

    printf("layer: %d, time: %.4f total:%.4f\n", conv_num, (double)(end-begin)/(double)CLOCKS_PER_SEC, time_accumulate);
        // printf("IP OUTPUT, %d\n", filter_offset);

        // printf("FILTER OFFSET AT T=2, %d\n", filter_offset);

        // for(i = 0; i<2*filter_offset; i++)
        // {
        //     printf("IP[%d]: %0.4f\n", i, C[i]);
        // }
    ...
    // write_outputs_to_file(conv_num, C, M, N, ldc);
        ...
        float *C, int ldc)
{
    ...
    if (is_fma_avx2() && !TA && !TB) {
        ...
        // for (t = 0; t < M; ++t) {
            if (!TA && !TB)
                gemm_nn(M, N, K, ALPHA, A, lda, B, lda, C, ldc);
            else if (TA && !TB)
