...
int recalculate_workspace_size(network *net)
{
    ...
    int workspace_core_adjusted = (int)ceil((float)workspace_size/(float)IP_BATCH_SIZE)*IP_BATCH_SIZE;

    // printf("WORKSPACE SIZE RECALC: %d ADJUSTED: %d\n", workspace_size, workspace_core_adjusted);
    static int call_count = 0;
    printf("CALLED RESIZE IN NETWORK %d\n", call_count++);
    cma_free((void*)net->workspace);
    net->workspace = (float*)cma_alloc(workspace_core_adjusted * 4, 1);
    assert(net->workspace != NULL);
