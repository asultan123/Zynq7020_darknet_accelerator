...
int main() {
    ...
    unsigned int num_of_batches = 10;
    unsigned int num_of_filters = 4;
    unsigned int rows_last_batch = 110;
    unsigned int output_offset = i_row;
    unsigned int last_batch = rows_last_batch*w_col;
    unsigned int image_size = ((i_row*num_of_batches*w_col)+last_batch);
    unsigned int weight_size = (w_col*num_of_filters);
    unsigned int dst_buf_size = (i_row*num_of_batches*num_of_filters+(rows_last_batch*num_of_filters));
    ...
    unsigned int* image_buff  = (unsigned int*)cma_alloc(image_size<<2, 0);
    unsigned int* weight_buff  = (unsigned int*)cma_alloc(weight_size<<2, 0);
    unsigned int* dst_buf = (unsigned int*)cma_alloc(dst_buf_size<<2, 0);
...
//HARDWARE
        ...
        dma_transaction((weight_buff+t*w_col), dst_buf, \
            w_col, i_row, \
        ...
        for(i = 0; i<num_of_batches; i++)
        {
            ...
            else
            {
                dma_transaction((image_buff+i*w_col), dst_buf+(i*output_offset), \
                    (i_row*w_col), i_row, \
                    IP_SEND_DATA, w_col, rows_last_batch);
            }
        }
